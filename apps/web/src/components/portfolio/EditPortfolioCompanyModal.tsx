...
interface EditPortfolioCompanyModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  company: PortfolioCompanyWithReport;
}

export function EditPortfolioCompanyModal({ open, onOpenChange, company }: EditPortfolioCompanyModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setPreview(company.preview || '');
      setSectors(company.sectors || []);
      setFounders(
        company.related_people 
          ? company.related_people.split(',').map(f => f.trim()) 
          : ['']
      );
      setInvestmentDate(
  ...
  const handleNext = () => {
    if (!companyName.trim()) {
      toast.error(t('modals.addCompany.nameRequired'));
      return;
    }
    setStep(2);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(t('modals.editCompany.success'));
      await queryClient.invalidateQueries({ queryKey: ['portfolio-company', company.id] });
      await queryClient.invalidateQueries({ queryKey: ['portfolio-company-with-report', company.id] });
      await queryClient.invalidateQueries({ queryKey: ['portfolio-companies'] });
      handleClose();
    } catch (error: any) {
      console.error('Error updating company:', error);
      toast.error(error.message || t('modals.editCompany.error'));
    } finally {
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-background rounded-xl shadow-2xl p-8 w-full max-w-md max-h-[90vh] overflow-y-auto relative">
        ...
        <h1 className="text-2xl font-semibold text-center text-foreground">
          {step === 1 ? t('modals.editCompany.title') : t('modals.addCompany.investmentDetails')}
        </h1>
        <p className="text-muted-foreground text-center mt-2 mb-8 text-sm">
          {step === 1 ? t('modals.addCompany.generalInfo') : t('modals.addCompany.step2of2')}
        </p>
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              <Label htmlFor="company-name">{t('modals.addCompany.companyName')}</Label>
              <Input
                id="company-name"
                value={companyName}
            ...
            {/* Domains (multi) */}
            <div className="space-y-2">
              <Label>{t('modals.editCompany.emailDomains')}</Label>
              <CompanyDomainsEditor companyId={company.id} />
              <p className="text-xs text-muted-foreground">{t('modals.editCompany.primaryDomainHelper')}</p>
            </div>

            {/* Description */}
            <div className="space-y-2">
              <Label htmlFor="description">{t('modals.addCompany.description')}</Label>
              <Textarea
                id="description"
                value={preview}
                onChange={(e) => setPreview(e.target.value)}
                placeholder={t('modals.addCompany.descriptionPlaceholder')}
                rows={3}
              />
            </div>
            ...
            {/* Sectors */}
            <div className="space-y-2">
              <Label>{t('modals.addCompany.sectors')}</Label>
              ...
              <div className="flex gap-2 mt-2">
                <Input
                  ...
                  placeholder={t('modals.addCompany.otherSector')}
                  className="flex-1"
            ...
            {/* Founders */}
            <div className="space-y-2">
              <Label>{t('modals.addCompany.founders')}</Label>
              {founders.map((founder, index) => (
                <div key={index} className="flex gap-2">
                  <Input
                    value={founder}
                    onChange={(e) => updateFounder(index, e.target.value)}
                    placeholder={t('modals.addCompany.founderPlaceholder')}
                    className="flex-1"
                  />
                  {founders.length > 1 && (
                    <Button
              ...
              >
                ...
                {t('modals.addCompany.addFounder')}
              </Button>
            </div>

            {/* Continue button */}
            <Button onClick={handleNext} className="w-full">
              {t('modals.addCompany.continue')}
            </Button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5">
            {/* Investment Date */}
            <div className="space-y-2">
              <Label>{t('modals.addCompany.investmentDate')}</Label>
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {investmentDate ? format(investmentDate, "PPP", { locale: dateLocale }) : t('modals.addCompany.selectDate')}
                  </Button>
                </PopoverTrigger>
            ...
            {/* Amount Invested */}
            <div className="space-y-2">
              <Label htmlFor="amount">{t('modals.addCompany.amountInvested')}</Label>
              <Input
                id="amount"
                type="number"
            ...
            {/* Investment Type */}
            <div className="space-y-2">
              <Label>{t('modals.addCompany.investmentType')}</Label>
              <Select value={investmentType} onValueChange={setInvestmentType}>
                <SelectTrigger>
                  <SelectValue placeholder={t('modals.addCompany.selectType')} />
                </SelectTrigger>
                <SelectContent>
            ...
            {/* Entry Valuation */}
            <div className="space-y-2">
              <Label htmlFor="valuation">{t('modals.addCompany.entryValuation')}</Label>
              <Input
                id="valuation"
                type="number"
            ...
            {ownershipPercentage && (
              <div className="p-3 bg-muted rounded-lg">
                <p className="text-sm text-muted-foreground">{t('modals.addCompany.calculatedOwnership')}</p>
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setStep(1)} className="flex-1">
                ...
                {t('modals.addCompany.back')}
              </Button>
              <Button onClick={handleSubmit} className="flex-1" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                    {t('modals.editCompany.saving')}
                  </>
                ) : (
                  t('modals.editCompany.save')
                )}
              </Button>
            </div>
          </div>
