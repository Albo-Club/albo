...
export function PortfolioCompanyInfoCard({ company }: PortfolioCompanyInfoCardProps) {
  ...
  const infoItems = [
    ...
    {
      icon: Globe,
      label: t('companyDetail.info.domain'),
      value: primaryDomain,
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg">{t('companyDetail.info.title')}</CardTitle>
          <Button variant="outline" size="sm" disabled>
            <FileText className="h-4 w-4 mr-2" />
            {t('companyDetail.info.lastReport')}
          </Button>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {company.sectors && company.sectors.length > 0 && (
          <div className="pt-4 border-t">
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
              ...
              <span>{t('companyDetail.info.sectors')}</span>
            </div>
        ...
        <div className="pt-4 border-t">
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
            ...
            <span>{t('companyDetail.info.coInvestors')}</span>
          </div>
          <p className="text-sm text-muted-foreground italic">
            {t('companyDetail.info.noCoInvestors')}
          </p>
        </div>

        {investmentDate && (
          <div className="pt-4 border-t text-xs text-muted-foreground">
            <p>{t('companyDetail.info.investmentDate')} {investmentDate}</p>
          </div>
        )}
      </CardContent>
    </Card>
