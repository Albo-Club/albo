...
export function DocumentPreviewModal({
  ...
  useEffect(() => {
    ...
    // If it's a text file with content and no storage file, show text directly
    if (document.text_content && !document.storage_path) {
    ...
    const loadFile = async () => {
      ...
      try {
        ...
        if (fileType === 'pdf') {
          const { data } = await downloadFromPortfolioStorage(
            document.storage_path!,
            document.source_bucket || 'portfolio-documents'
          );
          ...
          setFileUrl(url);
        } else if (buckets.includes('deck-files') && document.source_bucket === 'deck-files') {
  ...
  const renderContent = () => {
    ...
    if ((textContent || document.text_content) && !document.storage_path) {
      return (
    ...
    // PDF - use blob URL in object tag to avoid blocked third-party iframes
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-muted-foreground">{t('modals.reportViewer.noContent')}</p>
        <Button onClick={() => onDownload(document)}>
          <Download className="h-4 w-4 mr-2" />
          {t('common.download')}
        </Button>
