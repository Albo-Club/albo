...
export function PortfolioTable({ data }: PortfolioTableProps) {
  ...
  const columns = useMemo(() => getPortfolioColumns(t), [t]);
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex flex-1 items-center gap-2">
          ...
          {table.getColumn("sectors") && (
            <DataTableFacetedFilter
              ...
              title={t('portfolio.filters.sector')}
      ...
      <div className="shrink-0 py-2 px-2 text-xs text-muted-foreground">
        {table.getFilteredRowModel().rows.length} {t('portfolio.companiesCount')}
      </div>
    </div>
  );
}
