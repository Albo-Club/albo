...
export function CompanyAIBanner({ companyId }: CompanyAIBannerProps) {
    ...
    return (
      <div className="mb-6">
        <div className="rounded-2xl border bg-card p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold">{t('companyDetail.aiBanner.analyzing')}</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {t('companyDetail.aiBanner.analyzingDesc')}
            </p>
          </div>
        </div>
      </div>
  ...
  // ── Error ──
  if (analysisError || analysisStatus === "error") {
    return (
      <div className="flex items-center gap-3 p-4 rounded-2xl border mb-6 bg-card">
        <Sparkles className="h-5 w-5 text-muted-foreground flex-shrink-0" />
        <p className="text-sm text-muted-foreground flex-1">{t('common.error')}</p>
        <Button
          variant="outline"
          size="sm"
    ...
    return (
      <div className="flex items-center gap-3 p-4 rounded-2xl bg-muted/20 border border-dashed mb-6">
        ...
        <p className="text-sm text-muted-foreground flex-1">
          {t('companyDetail.aiBanner.noAnalysis')}
        </p>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          ...
          {t('companyDetail.aiBanner.analyze')}
        </Button>
      </div>
    );
  }
          ...
          <div className="flex items-start gap-4 lg:w-2/5">
            ...
            <div className="flex-1 min-w-0">
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom">
                      <p className="text-xs">{t('companyDetail.aiBanner.rerun')}</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              {goodPoints && goodPoints.length > 0 && (
                <div className="mt-2">
                  <span className="text-[10px] font-semibold uppercase tracking-wide text-emerald-600">{t('companyDetail.aiBanner.strengths')}</span>
              ...
              {badPoints && badPoints.length > 0 && (
                <div className="mt-1.5">
                  <span className="text-[10px] font-semibold uppercase tracking-wide text-red-600">{t('companyDetail.aiBanner.weaknesses')}</span>
                  <ul className="mt-0.5 space-y-0">
                    {badPoints.map((pt, i) => (
                      <li key={i} className="flex items-baseline gap-1.5">
