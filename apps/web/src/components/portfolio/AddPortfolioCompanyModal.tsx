...
export function AddPortfolioCompanyModal({ open, onClose }: AddPortfolioCompanyModalProps) {
  ...
  // Use fr locale for dates if language is French, otherwise default (en-US)
  ...
  const handleSubmit = async () => {
    if (!workspace?.id) {
      toast.error(t('modals.addCompany.noWorkspace'));
      return;
    }
    ...
    try {
      ...
      toast.success(t('modals.addCompany.success'));
      handleClose();
    } catch (error: any) {
      console.error('Error adding company:', error);
      toast.error(error.message || t('modals.addCompany.error'));
    } finally {
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-background rounded-xl shadow-2xl p-8 w-full max-w-md max-h-[90vh] overflow-y-auto relative">
        ...
        <h1 className="text-2xl font-semibold text-center text-foreground">
          {step === 1 ? t('modals.addCompany.title') : t('modals.addCompany.investmentDetails')}
        </h1>
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            {/* Domain */}
            <div className="space-y-2">
              <Label htmlFor="domain">{t('modals.addCompany.domain')}</Label>
              <Input
                id="domain"
                value={domain}
              ...
              />
              <p className="text-xs text-muted-foreground">{t('modals.addCompany.domainHelper')}</p>
            </div>
              ...
              <Button onClick={handleSubmit} className="flex-1" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
                    ...
                    {t('modals.addCompany.adding')}
                  </>
                ) : (
                  t('modals.addCompany.add')
                )}
              </Button>
            </div>
          </div>
