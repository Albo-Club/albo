...
const AnalysisLoader = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-10 space-y-8 bg-card rounded-xl shadow-sm border border-border">
      {/* Logo ALBO animé - lettres qui passent de gris à noir */}
      <div className="relative w-[220px] h-[80px]">
        ...
        {/* Animated letter overlays */}
        {letterSegments.map((segment, index) => (
          <svg 
            key={segment.id}
            width="220" 
            height="80" 
            ...
            style={{ 
              ...
              opacity: activeLetter === index ? 1 : 0,
            }}
          >
            <path
              fillRule="evenodd"
      ...
      <div className="flex space-x-2">
        {[0, 1, 2].map((i) => (
          <span
            key={i}
            className="w-2.5 h-2.5 rounded-full bg-foreground animate-bounce"
            style={{
      ...
      <p className="text-sm text-muted-foreground/60">
        {t('analysisLoader.elapsed')} {formatTime(elapsedSeconds)}
      </p>
    </div>
  );
};
