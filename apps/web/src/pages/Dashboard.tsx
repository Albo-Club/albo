  ...
  return (
    <div className="flex flex-col h-full w-full">
      <div className="shrink-0 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {deals.length > 0 ? `${deals.length} ${deals.length > 1 ? t('dashboard.deals_plural') : t('dashboard.deals')}` : t('dashboard.subtitle')}
          </p>
        </div>
        <Button onClick={() => navigate("/submit")}
        >
          <Plus className="mr-2 h-4 w-4" />
          {t('dashboard.submitDeal')}
        </Button>
      </div>
      ...
      {deals.length === 0 ? (
        <Card className="shrink-0 text-center py-12 mt-6">
          <CardContent>
            ...
            <h3 className="text-lg font-semibold mb-2">{t('dashboard.noDeal')}</h3>
            <p className="text-muted-foreground mb-4">{t('dashboard.noDealsDesc')}</p>
            <Button onClick={() => navigate("/submit")}
            >
              <Plus className="mr-2 h-4 w-4" />
              {t('dashboard.submitDeal')}
      ...
      <AlertDialog open={!!dealToDelete} onOpenChange={() => setDealToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('dashboard.deleteDealTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('dashboard.deleteDealDesc', { name: displayCompanyName(dealToDelete?.company_name) || "Sans nom" })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
