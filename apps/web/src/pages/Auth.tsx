        ...
        <div style={pageStyle}>
          <Card className="w-full max-w-md text-center" style={cardStyle}>
            <CardContent className="pt-6">
              ...
              <h2 className="text-xl font-bold mb-2" style={{ color: TEXT_PRIMARY }}>{t('auth.resetPassword.sent')}</h2>
              <p className="mb-4" style={{ color: TEXT_MUTED, fontSize: 14 }} dangerouslySetInnerHTML={{ __html: t('auth.resetPassword.sentDescription', { email: resetEmail }) }} />
      ...
      <div style={pageStyle}>
        <div className="w-full max-w-md">
          ...
          <Card style={cardStyle}>
            <CardHeader>
              <CardTitle style={{ color: TEXT_PRIMARY }}>{t('auth.resetPassword.title')}</CardTitle>
            ...
            <CardContent>
              <form onSubmit={handleResetPassword} className="space-y-4">
                <div>
                  <Label htmlFor="reset-email" style={{ color: TEXT_PRIMARY }}>{t('auth.login.email')}</Label>
                ...
                <Button type="submit" className="w-full" style={btnPrimary} disabled={resetLoading || !resetEmail}>
                  {resetLoading ? t('auth.resetPassword.loading') : t('auth.resetPassword.submit')}
                </Button>
  ...
  return (
    <div style={pageStyle}>
      <div className="w-full max-w-md">
        ...
        <Card style={cardStyle}>
          <CardHeader>
            <CardTitle style={{ color: TEXT_PRIMARY }}>{t('auth.login.title')}</CardTitle>
            <CardDescription style={{ color: TEXT_MUTED }}>{t('auth.login.subtitle')}</CardDescription>
          </CardHeader>
          <CardContent>
            <Button
              variant="outline"
            ...
            <div className="relative mb-6">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span style={{ background: '#fff', padding: '0 8px', color: TEXT_MUTED }}>{t('auth.login.withEmail')}</span>
              </div>
            </div>

            <Tabs defaultValue="signin">
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="signin">{t('auth.login.tabSignIn')}</TabsTrigger>
              ...
              <TabsContent value="signin">
                <form onSubmit={handleSignIn} className="space-y-4">
                  <div>
                    <Label htmlFor="signin-email" style={{ color: TEXT_PRIMARY }}>{t('auth.login.email')}</Label>
                  ...
                  <div>
                    <Label htmlFor="signin-password" style={{ color: TEXT_PRIMARY }}>{t('auth.login.password')}</Label>
              ...
              <TabsContent value="signup">
                {signUpSent ? (
                  <div className="text-center py-4">
                    ...
                    <h3 className="font-semibold mb-2" style={{ color: TEXT_PRIMARY }}>{t('auth.signup.sent')}</h3>
                    <p className="text-sm mb-4" style={{ color: TEXT_MUTED }}>
                      {t('auth.signup.sentDescription')}
                    </p>
                    <Button variant="outline" onClick={() => { setSignUpSent(false); setSignUpEmail(''); }}>
                      {t('auth.signup.retryOther')}
                    </Button>
                  </div>
                ) : (
                  <form onSubmit={handleSignUp} className="space-y-4">
                    <div>
                      <Label htmlFor="signup-email" style={{ color: TEXT_PRIMARY }}>{t('auth.signup.email')}</Label>
