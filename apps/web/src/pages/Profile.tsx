...
import { Loader2, User, Mail, Plug, RefreshCw, Unplug, Server, AlertCircle, LogOut, Clock, Globe } from 'lucide-react';
...
import { ResyncEmailDialog } from '@/components/email/ResyncEmailDialog';
...
export default function Profile() {
  ...
  const [formData, setFormData] = useState({
    ...
    preferred_language: 'fr' as string,
  });
  ...
  // États pour la modal de resync
  const [resyncDialogOpen, setResyncDialogOpen] = useState(false);
  const [resyncAccount, setResyncAccount] = useState<ConnectedAccount | null>(null);
  ...
  return (
    <div className="container max-w-3xl py-8 space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select 
                ...
                onValueChange={(val) => setFormData(prev => ({ ...prev, preferred_language: val }))}
              >
                    ...
                    {connectedAccounts.map((account) => (
                      ...
                      >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              {account.last_synced_at && (
                                <span className="text-xs text-muted-foreground">
                                  · Dernière sync : {timeAgo(account.last_synced_at)}
                                </span>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {(account.status === 'active' || account.status === 'syncing' || account.status === 'sync_error') && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => {
                                setResyncAccount(account);
      ...
      {/* Modal de resync */}
      {resyncAccount && workspace && (
        <ResyncEmailDialog
          open={resyncDialogOpen}
          onOpenChange={setResyncDialogOpen}
          workspaceId={workspace.id}
          providerAccountId={resyncAccount.provider_account_id}
        />
      )}
