...
export default function PortfolioCompanyDetail() {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
        ...
        <div className="lg:col-span-3">
          <div className="sticky top-4">
            <PortfolioCompanyOverview company={company} latestReport={latestReport} />
          </div>
        </div>
      </div>
    </div>
  ...
  const emailsContent = (
    <CompanyEmailsTab companyId={company.id} companyName={company.company_name} domain={company.domain} />
  );
  const metricsContent = <MetricsDashboard companyId={company.id} />;
  ...
  return (
    <div className="space-y-6">
      ...
      />
      <DealTabs overviewContent={overviewContent} emailsContent={emailsContent} foldersContent={documentsContent} metricsContent={metricsContent} />
      <UploadReportModal open={uploadModalOpen} onOpenChange={setUploadModalOpen} companyId={company.id} companyName={company.company_name} />
      <ReportDetailSheet open={!!detailReportId} onOpenChange={(open) => { if (!open) setDetailReportId(null); }} reportId={detailReportId} />
    </div>
  );
}
