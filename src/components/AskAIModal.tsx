...
import { Send, Bot, User, Sparkles, TrendingUp, GitCompare, PieChart, Layers } from "lucide-react";
...
export function AskAIModal({ isOpen, onOpenChange }: AskAIModalProps) {
  const { user } = useAuth();
  ...
  const sendMessage = async (messageText?: string) => {
    ...
    try {
      ...
      if (!activeConversationId) {
        const { data: newConv, error: convError } = await supabase
          ...
          .insert({
            user_id: user.id,
            deal_id: null,
            title: `Ask AI - ${new Date().toLocaleDateString("fr-FR")}`,
            source: "chat",
          })
          .select()
          .single();
      ...
      // 3. Call N8N webhook WITHOUT deal_id (global query)
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8">
              <div className="text-center mb-8">
                ...
                <p className="text-muted-foreground max-w-md">
                  ...
                  Evaluate market, financials, team, and innovation.
                </p>
              </div>
